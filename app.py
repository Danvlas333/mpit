    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/performance":
            if not user:
                ...
                return
            ensure_user_storage(self.data_root, user["storage_key"])
            self._send_json(200, {"ok": True, "performance": load_student_performance(self.data_root, user["storage_key"])})
            return
        ...
        if path == "/api/grades":
            if not user:
                ...
                return
            ensure_user_storage(self.data_root, user["storage_key"])
            self._send_json(200, {"ok": True, "grades": load_student_grades(self.data_root, user["storage_key"])})
            return
